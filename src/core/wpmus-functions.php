...
function wpmus_maybesync_newuser($user_login)
{
    ...
    // Check if the sync is required.
    if ($wpmus_newUserSync == 'yes') {
        // Choose the appropriate function to get user data based on availability.
        if (function_exists('get_user_by'))
            $userdata = get_user_by('login', $user_login);
        else
            $userdata = get_userdatabylogin($user_login);

        // Sync the user if userdata is valid and specific user meta condition is not met.
        if ($userdata !== false && get_user_meta($userdata->ID, 'msum_has_caps', true) != 'true') {
            wpmus_sync_newuser($userdata->ID);
        }
    } else {
        // If $wpmus_newUserSync is not 'yes', output 'maybe' and redirect.
        echo "maybe";
